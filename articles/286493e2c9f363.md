...
X11でウィンドウマネージャーを開発する際、左右のAltキー（Alt_L、Alt_R）を区別してショートカットを設定したいことがあります。

例えば、「Alt_L + m」でChromeを起動、「Alt_R + m」でTerminalを起動、といった使い分けです。

しかし、X11のキーイベントだけでは左右のAltを区別できません。この記事では、`QueryKeymap`を使った解決方法を紹介します。

## この記事で分かること

- ✅ X11で左右のAltキーを区別できない理由（ModMask、KeyCode、KeySymの概念）
- ✅ QueryKeymapを使った実装方法（Rust + x11rb）
- ✅ ビットベクトルでキー状態をチェックする仕組み
