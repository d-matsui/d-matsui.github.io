...
X11 における Window Manager は、クライアントがサーバーに送信したリクエストを、Xサーバーにリダイレクトしてもらうことで、タイル型のウィンドウ管理などを実現します。

X11 Protocol は仕様として主に個々のリクエスト・イベントについて説明しているため、クライアント・サーバー・Window Manager 間でやりとりされるリクエストやイベントがまとまってる訳ではありません。
